...
`sku.capacity` values are also preserved unless `--sync-capacity` is provided.
"""
...
def apply_model_to_entry(
    ...
    *,
    ...
    if sync_capacity or "capacity" not in sku:
        set_if_changed(sku, "capacity", model.capacity, changes)
